...
class Map(object):
    ...
    * https://tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png [grayscale]
    ...
    def from_pixels(self, px, py):
        """Convert from pixels in the image to a geographical coordinate"""
        x = self.xmin + float(px)/self.tilesize
        y = self.ymin + float(py)/self.tilesize
